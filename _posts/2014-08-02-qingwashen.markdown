...
昆生所在的湖北由$N$个县组成，标号为$1$到$N$。这些县由$M$条有向道路串联起来，出于一些不知名的目的，这张图丧心病狂的构成了一张有向无环图（DAG）。昆生决定在这张图中选取一条有向路径（可以只包含一个点），这样他在经过每个县里都能买到一个具有这个县标号的纪念品。为了讨十娘开心，昆生希望这些纪念品标号的乘积是一个完全平方数。

你的任务是统计有多少条路径满足有求。

##输入
输入第$1$行包含两个整数$N$，$M$。<br />
第$2$到$M+1$行每行包含三个整数$U_i$，$V_i$，表示有一条$U_i$与$V_i$的**单向道路**。<br />
...
##样例解释
4条路径分别是(1->1)，(4->4)，(2->6)，(4->6)。

##数据说明

对于$30\%$的数据，$N\leq 10。$<br />
对于$100\%$的数据，$1\leq N\leq 90，0\leq M\leq 8000。$<br />
